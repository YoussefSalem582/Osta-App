...
/// Paginated 2-column product grid driven by the ambient [ShopListCubit].
...
class _ShopProductGridState extends State<ShopProductGrid> {
  ...
  void _onScroll() {
    if (_scroll.position.pixels >= _scroll.position.maxScrollExtent - 400) {
      unawaited(context.read<ShopListCubit>().loadMore());
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<ShopListCubit, ShopListState>(
      builder: (context, state) {
        ...
        if (state.status == ShopListStatus.error) {
          return ErrorState(
            ...
            onRetry: () => context.read<ShopListCubit>().load(),
          );
        }
