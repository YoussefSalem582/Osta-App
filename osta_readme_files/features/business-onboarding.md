...
`lib/features/business/` is currently a **stub**: only empty directories (`bookings/`, `dashboard/`, `services/`, `team/`, `wallet/`) with no Dart files, and no onboarding folder yet. Everything below is planned per epic [app #53](https://github.com/YoussefSalem582/Osta-App/issues/53).

> ‏مجلد `lib/features/business/` **مبدئي** حاليًا: مجرد مجلدات فارغة (`bookings/`، `dashboard/`، `services/`، `team/`، `wallet/`) بلا أي ملفات Dart، ولا يوجد مجلد للانضمام بعد. كل ما يلي مُخطَّط وفق المهمة [app #53](https://github.com/YoussefSalem582/Osta-App/issues/53).
