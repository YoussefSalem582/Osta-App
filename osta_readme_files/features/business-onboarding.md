...
`lib/features/business/onboarding/` is **wired** (2026-07-16): presentation (3 wizard screens + widgets including `MapPinPickerSheet`) + data (`BusinessOnboardingRepository`, `CatalogPreset`, `BusinessProfileInput`) + `BusinessOnboardingCubit`. A `ShellRoute` shares one cubit across `/provider-onboarding` → `/business-identity` → `/business-catalog`. Completion is persisted as `session_business_onboarded` in `SessionStore`. Custom-service entry and social business signup remain stubs.

> ‏`lib/features/business/onboarding/` **موصول** (2026-07-16): طبقة العرض + البيانات + الـ Cubit. يشارك `ShellRoute` cubitًا واحدًا عبر خطوات المعالج. اكتمال المعالج يُحفظ في `SessionStore`. إدخال الخدمة المخصّصة والتسجيل الاجتماعي ما زالا stubs.
...
- **Catalog step**: loads 12 seeded presets, requires ≥1, bulk-attaches via `POST /business/catalog`.
- **DI**: manual `get_it` — `BusinessOnboardingRepository` (lazy singleton) + `BusinessOnboardingCubit` (factory) in `configureDependencies()`.
- **Routing**: authenticated `business` + `!businessOnboarded` → identity → catalog wizard; Activate → persist flag → `/business` shell ([app #34](https://github.com/YoussefSalem582/Osta-App/issues/34)). `/provider-onboarding` intro is routable but no longer the forced entry (merchants already saw the logged-out carousel).
