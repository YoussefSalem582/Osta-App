...
## 2026-07-18 — First UI on the newly-wired data layer (Notifications + Addresses)

The Jul-17 change wired the data layer to every endpoint but built **no UI** — by scope. An **endpoint→UI audit** (14-area fan-out, each repo method traced repo→cubit→page→route, then an adversarial verify pass) confirmed the gap: ~49 repo methods across 14 areas have **no reachable UI** — the presentation layer is largely static mockups, and the profile/More tab still had three dead `onTap: () {}` rows (Addresses, My Store, Notifications). 27 screens/wirings are needed to make every orphaned endpoint reachable.

**This change ships the two cleanest shared verticals** — both entered from those already-scaffolded dead rows, both with repo + model + l10n already in place, so the diff is a cubit + page + route + one-line row wiring each.

**Notifications** — `NotificationsCubit` + `NotificationsPage`, route `/notifications`. In-app inbox over `GET /notifications` (per-page 50): newest-first, unread rows bold with a brand-green dot, tap-an-unread fires `POST /notifications/{id}/read` and swaps the row in place (no reload), an app-bar "mark all read" loops the same endpoint over the unread set, pull-to-refresh + `EmptyState`/`ErrorState`.

**Addresses** — `AddressCubit` + `AddressesScreen` + self-contained `AddressFormScreen`, route `/addresses`. Full `/me/addresses` CRUD: card list with a default badge and delete-with-confirm (`GET`/`DELETE`), create/edit form (`POST`/`PUT`) with a home/work/other `AppSegmentedToggle` and the full `AddressResource` field set. Because `PUT` is full-replace, the edit form carries `latitude`/`longitude` through unseen so text edits don't null the map pin server-side.

**Wiring & house style.** The two dead profile rows now `context.push` the new routes. Cubits are plain `Cubit<State>` constructed in the page's `BlocProvider` (not DI, matching `ProfileCubit`/`MyProductsCubit`); states are `Equatable` sub-class families; screens reuse `AppTopBar`/`AppTextField`/`AppButton`/`AppConfirmDialog`/`status_states`/`AppToaster`. ~30 l10n keys added to both ARBs + `flutter gen-l10n`. No new dependencies.

**Verification.** `flutter analyze` clean on every new/changed file (`notifications/`, `profile/presentation/`, `core/router/`, `l10n/`); the 29 project-wide infos that remain are all pre-existing in untouched files. `dart format` clean.

**Follow-on same day — customer booking flow.** `BookingsCubit` + `bookings_state` wire `MyBookingsScreen` to `GET /bookings` (upcoming + past loaded in parallel, tabs switch client-side; API `Booking`→`BookingItem` mapper since the list endpoint doesn't eager-load center/items). `BookingCard` now pushes `/booking-status` with the booking id. `BookingDetailCubit` + `booking_detail_state` wire `LiveBookingScreen` to `GET /bookings/{id}` and the three lifecycle mutations — confirm (`POST`), reschedule (`PATCH`, native date+time pickers), cancel (`POST`, confirm dialog) — state-gated by `status`, run under a blocking overlay, falling back to the last loaded booking on failure. 12 more l10n keys. `flutter analyze` **No issues** on the booking + router tree. Booking **create** (`POST /bookings`) stays pending: it needs the center-detail + availability entry to supply center/service/slot.

**Follow-on same day — customer booking loop closed (center detail + create).** The map marker sheet's "Book"/"Details" buttons (both a coming-soon toast) now push `CenterDetailPage` (`/center-detail`). `CenterDetailCubit` loads `GET /centers/{id}` + `/services` + `/centers/{id}/reviews` together (services/reviews degrade to empty, never blank the page); a write-review sheet posts `POST /centers/{id}/reviews`. Its Book CTA opens `BookingCreateScreen` (`/booking-create`): `BookingCreateCubit` runs service multi-select → `DaySelector` → per-day `GET /centers/{id}/availability` slots (`TimeSlotChip`) → `BookingBottomBar` total → `POST /bookings`, then `pushReplacement` to the live status screen. Reuses the existing booking-date widgets rather than the fixture-driven `BookingDateScreen`. ~22 l10n keys (incl. `currencyEgp`). `flutter analyze` **No issues** on the map + booking + router tree. This closes the customer path end-to-end (browse → center → review/book → slot → create → live → confirm/reschedule/cancel); 6 more endpoints wired (center show/services/availability, center reviews read+write, booking create).

**Follow-on same day — business booking queue.** The provider `Bookings` screen (business shell calendar action) was one static card; now `BusinessBookingsCubit` + `business_bookings_state` drive a live feed over `GET /business/bookings` with a horizontal status filter, and each card runs the transition its state permits — accept (`PATCH .../accept`), reject (`.../reject` + required-reason dialog), start/complete (`.../status`), and an assign-mechanic sheet that reads `GET /business/mechanics` and posts `.../assign-roster-mechanic`. Actions run under a blocking overlay then reload. Rewrote the static screen, deleted three orphaned mockup widgets (`custom_row`/`driver_title`/`selected_type`), added `assign_mechanic_sheet`; ~13 l10n keys. `flutter analyze` **No issues** on the business-bookings tree. 6 more endpoints wired (business list + 4 transitions + mechanics read); user-type `assignMechanic` stays repo-only.

**Follow-on same day — business dashboard counts.** `DashboardCubit` wires `BoardScreen` to `GET /business/dashboard`: the hero shows real revenue and three tiles show today/pending/completed (the model's exact fields). Dropped the hardcoded `4.250 EGP`, the fake "12% from yesterday", and two unbacked status tiles; loading shows `…`, error `—`. `dashboardToday` key. `flutter analyze` **No issues**. The `Customer()` preview list stays a placeholder (real queue = Bookings screen); capacity editor (`PUT`) pending.

**Follow-on same day — business services CRUD.** Added `BusinessServiceRepo.create` (self-contained `POST /business/services`), then `ServicesCubit` + `services_state` driving the Catalog → Services segment over `GET /business/services`: real `ServiceToggleCard`s with the switch wired to `PUT .../{id}` `is_active`, remove to `DELETE .../{id}` (confirm dialog), card-tap → `ServiceFormScreen` (edit), `+` → create — a validated form (name/price/price-type toggle/category/duration/description/active). Toggle+delete run under an overlay then reload. ~22 l10n keys. `flutter analyze` **No issues**. Offers segment stays a placeholder (promotions CRUD pending).

**Follow-on same day — Home visual pass (Book hero + location empty state).** The feed read weak with no location fix: the "Book a service" button clipped to "Book a" and greyed out disabled, and the page was mostly empty. Rebuilt the Book card as a full-width branded hero (green, icon, title/subtitle, direction-aware arrow, whole-card tap) that's always actionable — opens the nearest center or re-requests a fix. `HomeBloc` now tracks `locationDenied`, so the nearby section shows a "Find centers near you / Enable location" card instead of vanishing. Follow-up polish: reusable `HomeEmptyCard` gives every rail a "no data" state (nearby: enable-location or "no centers nearby"; shop: "no products yet") so sections keep their header instead of dropping out, "See all" hides when empty, product price is brand-green bold, center rating bolder, and the Home app bar sets `centerTitle: false` (the theme default `true` was clustering the bell+greeting in the middle) so it spans and mirrors correctly LTR/RTL. 5 l10n keys. Analyze clean, suite 145/145. Then: rail tiles were clipping (fixed 210px rail < tile height under Cairo's tall metrics) → switched the rail to a content-sized `SingleChildScrollView`+`Row` (tile column `mainAxisSize.min`) so it grows to the tallest tile, thumbnail 100→120; and swapped the header to greeting/name-leading + bell-trailing (name ellipsizes, still RTL-correct). Then the Home shop rail was switched to reuse the Store screen's `ProductGridCard` (same bordered card, `ProductImage`, bold primary price, owner line) — each rail tile boxed to the Store grid's 0.72 aspect — so a product looks identical in both surfaces; the unused home `ProductCard` was deleted (`HomeTile` stays for centers), and the committed `home_rail_test` updated to the content-sized structure. (The earlier double-header screenshot was a stale build — the greeting is the app bar after rebuild.)

**Follow-on same day — customer Home UI/UX refactor.** With Home now data-backed, refreshed the surface: `Skeletonizer` shimmer over stand-in rails on first load (replacing the thin progress bar), real `CachedNetworkImage` thumbnails with distinct fallback icons, and every control wired — nearby tile → centerDetail, product tile → productDetail, active-booking hero → live status, "Book a service" → nearest center (disabled when none), shop "See all" → browse, header bell → notifications. Time-aware greeting (morning/afternoon/evening) replaces the fixed one. New optional params (`HomeTile.onTap/imageUrl/placeholderIcon`, `HomeRail.onSeeAll`, `ActiveBookingCard.onTap`, `BookServiceCard.onBook`); 4 l10n keys (3 greeting variants + seeAll) replacing `homeGreeting`; reuses the already-present skeletonizer/cached_network_image. Analyze clean, full suite 145/145. Then the **greeting header became the Home tab's app bar** (replacing the shell's logo/"Home"/overflow bar): new `AppBottomNavItem.chromeless` makes `RoleShell` skip its `AppTopBar` for that tab, and `HomePage` supplies its own themed `AppBar` (correct status-bar icons) hosting the header as title, dropped from the scrolling feed; switch-role/sign-out remain on the other tabs + More.

**Follow-on same day — 10 Cubits → event-driven Blocs.** For consistency with the app's existing `MapBloc`, this session's ten new Cubits were converted to `Bloc<Event, State>` (part-file idiom: `bloc/<name>_bloc.dart` + `_event.dart` + `_state.dart`). One event per public method, feature-prefixed names (globally unique — the structure test forbids duplicate class names), handler bodies verbatim; constructor auto-loads became `…Started` events fired at `BlocProvider` create. The one non-mechanical case — methods whose return value the page consumed (mainly `BookingCreateCubit.submit()` → `Future<Booking?>` awaited to `pushReplacement` to live status) — moved the result into one-shot state fields (`createdBooking`/`submitFailed`) driven by a `BlocListener`, preserving navigation + toasts exactly. Converted: notifications, addresses, customer booking (list/detail/create), center-detail, home, business bookings, dashboard, services; pages rewired to `add(event)` + `BlocBuilder/Consumer<XBloc>`; awaited-`load()` pull-to-refresh now awaits `bloc.stream.firstWhere(...)`. Also fixed two onboarding test fakes missing a `fetchProfile()` override (from the earlier business-profile work, surfaced by the full suite). Ran as an 8-agent workflow (one per cubit-directory) + global verify. No behavior/data/DI change. Full suite 145/145, analyze 0 errors/0 warnings, structure tests pass. The two business form screens stay StatefulWidget; pre-existing `ProfileCubit`/`BusinessOnboardingCubit` untouched.

**Follow-on same day — customer Home tab wired to the backend.** Home was fixture-only (`home_fixtures.dart`). A new `HomeCubit` loads four feeds concurrently — `GET /me` (header name, seeded from the profile cache for instant paint), `GET /bookings` + `GET /bookings/{id}` (the current non-terminal booking, re-fetched for the service/center names the list omits), `GET /centers/nearby` (from the device fix), `GET /products` (shop rail) — and degrades per-section (a failed feed empties only its rail). Active-booking hero shows only when there's a live booking; rails hide when empty; pull-to-refresh added; `home_fixtures.dart` deleted and every section widget made data-driven. Reuses `ProfileRepo`/`BookingRepo`/`CentersRepository`/`ShopRepo` — no new endpoint, dependency, or l10n key. `flutter analyze` clean; structure tests pass. The Book CTA, header bell, and rail-card taps stay nav-only stubs (separate polish).

**Follow-on same day — reverse-geocode auto-fill for address entry.** Adding a business address required a pin *and* typing city/street/district. Now dropping the pin reverse-geocodes it (on-device `geocoding` package — no API key) and fills any of those left blank, never overwriting typed text; failures leave the form manual. Behind a best-effort `ReverseGeocoder` (`core/services/reverse_geocoder.dart`), wired into the onboarding identity step, `BusinessProfileScreen`, and `BusinessAddressScreen`. Backend needs no change — `UpdateBusinessProfileRequest` and the customer `Store/UpdateAddressRequest` already make every field optional (customer address requires only `label`), so the friction was manual typing, not validation. The customer delivery-address form has a deeper gap (no pin picker at all — it only carries lat/lng through on edit); fixing it needs the picker widget moved out of `business/` into a shared home, spawned as a separate task.

**Follow-on same day — graceful fallback when the profile GET isn't deployed.** The live server ships only `PUT /business/profile`; the read-back `GET` (in source, not deployed) 405s, and no other deployed endpoint returns the center address (`/me`/register `UserResource` has no center fields; `/business/dashboard` is counts+revenue). A 405 mapped to a generic `ServerException`, so both new screens hit their error wall. Added `MethodNotAllowedException` (405) to the sealed `ApiException` set; `BusinessProfileScreen` + `BusinessAddressScreen` catch it (+ `NotFoundException`) and start a **blank editable form** instead — the partial `PUT` save works now, prefill auto-enables once the `GET` deploys. Test locks the 405 mapping.

**Follow-on same day — business address row + focused editor.** The provider profile's one account row ("Business profile") opened the full info+logo+pin form; added a business-only **"Business address"** row beneath it opening `BusinessAddressScreen` (route `/business/address`) — map pin (`LocationPickerCard`/`MapPinPickerSheet`) + city/street/district only. Same `GET /business/profile` read-back (`fetchProfile`) and same partial `PUT` (`updateProfile`): `toJson` omits nulls + backend rules are `sometimes`, so an address-only save leaves trade name/type/logo intact. Route added to the redirect allow-list; 5 l10n keys (EN + AR). No backend change, no new dependency. `flutter analyze` **No issues**; structure + redirect tests 22/22.

**Follow-on same day — map "nearby only" toggle.** The customer map only ever called `GET /centers/nearby`, hard-capped at the backend's 50 km radius (`centers.nearby.max_radius_meters`), so a customer in a sparse area hit "No centers found" with no escape hatch. Added a filter sheet (`map_filter_sheet.dart`, opened by an `Icons.tune` button in the search row that wears a dot badge when a non-default filter is active) with a single **"Nearby only"** switch, on by default. Off, a blank search routes through `GET /centers/search` with no `q` — `SearchCentersAction` is non-geospatial and returns every active center ordered by rating — so distant centers still drop markers. `MapState.nearbyOnly` + `NearbyOnlyToggled`; `_load` uses nearby only for `query.isEmpty && nearbyOnly`, else search (search already handled non-empty queries, so the change is one condition). Reuses the existing `CentersRepository.search` — **no backend change, no new dependency**. 4 l10n keys (EN + AR). `flutter analyze` **No issues** on the map tree; `map_bloc_test` 21/21 with 2 new toggle cases (off → search-not-nearby, same-value → no reload).

**Follow-on same day — Technicians, Offers, and Garage maintenance wired.** Three more of the orphaned screens above got real UI, one at a time via a 3-agent sequential workflow (each agent mirroring an existing sibling pattern in its own feature directory). **Technicians** (`/business/technicians`, was three hardcoded "Mahmoud" cards, dead add/edit buttons): `MechanicsBloc` (load / active-toggle / delete over `MechanicRepo`, copying `ServicesBloc`'s `_load`/`_act` structure) + a new `MechanicFormScreen` (name/specialty required, phone optional, active switch) — `TechScreen` keeps its original card visual language but now renders `state.mechanics`, its add icon and add-tile both open the form, and each row deletes behind `AppConfirmDialog`. **Offers** (the Catalog tab's second segment, was a single static `DiscountPromotionBanner`): `PromotionsBloc` over `PromotionRepo` (list/active-toggle/delete) added alongside `ServicesBloc` in a `MultiBlocProvider` so both load eagerly; a new `PromotionRow` list widget (title, discount formatted percent-off or EGP-off, redemption count, active toggle, edit/delete) and `PromotionFormScreen` (title, percent/fixed `AppSegmentedToggle`, value, start/end date pickers, code, max redemptions, active switch) replace `_offersPlaceholder()` — the Offers segment is now full CRUD like Services. **Garage maintenance** (no screen existed; `MaintenanceRepo`/`MaintenanceRecord` sat unused): a new `MaintenanceCubit` (one instance per vehicle, mirroring `GarageCubit`'s per-action loading/success/error sub-states) over `MaintenanceRepo.history`/`addRecord`, a `MaintenanceScreen` (history list + empty/loading/error states) reached by tapping a vehicle card in `MyGarageScreen` (`VehicleCard` previously had no tap handler at all), and an `AddMaintenanceRecordSheet` modal form (expense type, performed-at date capped to today-or-earlier, description, mileage, cost). One new route, `/maintenance`, passes the vehicle id via `state.extra` matching the `bookingStatus`/`productDetail` pattern. Deliberate scope cuts: PDF export and receipt-file upload — the repo supports both, neither has UI yet. ~60 l10n keys (EN + AR) across the three features, no new dependencies, no DI changes (all three repos are static-methods classes like their siblings). `flutter analyze` **0 errors/0 warnings** repo-wide (29 pre-existing infos, none in touched files); full suite **145/145** — no new tests added, matching this codebase's existing no-test-precedent for a Bloc/Cubit built on a static repo (`ServicesBloc`/`GarageCubit` have none either).

**Remaining (mapped, pending).** ~9 orphaned screens: customer wallet/payment (no backend endpoint at all yet); business wallet (empty stub, no backend); capacity editor (`PUT /business/dashboard` capacity field); vehicle detail/edit; shop reviews; social sign-in (Google/Apple buttons exist, dead); maintenance PDF export + receipt upload (repo ready, no UI). All except the two wallets have a live data layer already.
