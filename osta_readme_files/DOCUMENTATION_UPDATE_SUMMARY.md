...
## 2026-07-08 — Auth screens UI/UX refactor + branded logo (AuthScaffold)

Refactored the four auth screens' UI/UX and put the app logo on every one of them. New shared `AuthScaffold` (`lib/features/auth/presentation/widgets/auth_scaffold.dart`) renders a collapsing brand-green `SliverAppBar` whose hero band holds the white OSTA logo (back button pinned over it, subtle shadow once content scrolls under), a bold centered title, and an optional subtitle; all four screens (auth-choose, login/register, forgot, reset) now build from it, so the logo carries through the whole flow (was chooser-only). The chooser shows the full lockup (`AppImages.fullLogo`), the inner screens the smaller wordmark (`AppImages.logo`) — both white assets that read on `AppColors.brandGreen`, enlarged for presence (hero 180, inner 120). Form fields are wrapped in an elevated `AppCard`. The hardcoded, dark-mode-broken `socialButton` was replaced by the tokenized `AppButton` (secondary + icon) at both call sites and its file deleted; raw `TextStyle(color: error)` form errors became a themed `AuthFormError`. No new strings/deps/routes; success sub-views stay icon-led. Analyze clean; all 107 tests pass.

> ‏أُعيد تصميم واجهة/تجربة شاشات المصادقة الأربع مع وضع شعار التطبيق على كلٍّ منها. يعرض `AuthScaffold` المشترك الجديد شريطًا علويًا أخضر يحمل شعار OSTA الأبيض أسفل `AppBar` شفّاف يحوي زر الرجوع فقط، ثم عنوانًا عريضًا متوسّطًا وعنوانًا فرعيًا اختياريًا؛ وتُبنى الشاشات الأربع (auth-choose، الدخول/التسجيل، نسيت كلمة المرور، إعادة التعيين) منه، فامتدّ الشعار عبر التدفّق كله (كان على شاشة الاختيار فقط). تعرض شاشة الاختيار الشعار الكامل (`AppImages.fullLogo`)، والشاشات الداخلية النسخة الأصغر (`AppImages.logo`) — وكلاهما أبيض يظهر على `AppColors.brandGreen`. وأصبحت الحقول داخل `AppCard` بارز. واستُبدل `socialButton` المكسور في الوضع الداكن بـ `AppButton` (ثانوي + أيقونة) وحُذف ملفه، وصارت أخطاء النماذج عبر `AuthFormError` موحّد. بلا نصوص/حزم/مسارات جديدة؛ وتبقى شاشات النجاح بالأيقونة. التحليل نظيف وكل الاختبارات (107) تنجح.

Touched: `lib/features/auth/presentation/widgets/{auth_scaffold,auth_form_error}.dart` (new), `lib/features/auth/presentation/{auth_choose_page,auth_page,forgot_password_page,reset_password_page}.dart`, deleted `lib/features/onboarding/widget/social_button.dart`, `CHANGELOG.md`, `CURRENT_STATUS.md`.
