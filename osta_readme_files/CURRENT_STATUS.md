...
> **Last Updated:** Jul 8, 2026 — **auth screens UI/UX refactor + branded logo**: a new shared `AuthScaffold` (collapsing brand-green `SliverAppBar` hero band + white OSTA logo, back button pinned over it, bold centered title + optional subtitle) now backs **all four** auth screens (auth-choose, login/register, forgot, reset), so the logo carries through the whole flow (was chooser-only) — full lockup on the chooser (h180), larger wordmark inside (h120). Form fields are wrapped in an `AppCard`; the hardcoded dark-mode-broken `socialButton` was replaced by the tokenized `AppButton` (secondary + icon) and deleted; raw error text → themed `AuthFormError`. No new strings/deps/routes; analyze clean, 107 tests pass. Detail: [`DOCUMENTATION_UPDATE_SUMMARY.md`](DOCUMENTATION_UPDATE_SUMMARY.md).
